...
### 🔧 修复 — 额度跑干的号不再赖在池里刷 400（线上单日 3005 次硬失败）

额度耗尽的自动下线**一次都没生效过**。判定写的是 `402 && reason 命中`，但上游早已改
用 400 + `ServiceQuotaExceededException` / `OVERAGE_REQUEST_LIMIT_EXCEEDED` 下发——
线上 30 天里 402 一次没出现过，`quota_exhausted` 这个 outcome 也从未被记录。

于是跑干的号一直是「可用」状态被继续选中，每次都撞进「400 一律终止」分支：不重试、
不换号、不计失败，直接把错误抛给客户端。9 月 7 日一天里两张号（余额均为 0/2000）
就这样产生了 **3005 次本可故障转移的硬失败**。

- 额度耗尽判定改为**只认响应体 `reason`，不绑状态码**，并前置到 400 分支之前。
  状态码是上游可以随时改的实现细节，`reason` 才是稳定契约。
- 降级链同样处理：备用桶报额度耗尽时禁号换号，而不是当成终态请求错误。额度按账号算
  不按桶算，换桶不会有别的结果。
- MCP 路径同样去掉 402 门槛。

命中后行为回到设计意图：当场禁号（`disableReason=QuotaExceeded`、落
`quotaExhaustedAt`，面板可与手动禁用区分、补货判定能识别），并把这次请求转移到还有
额度的号上。

### 🛡️ 兜底 — 认不出来的错误也不能让号无限烧请求

上面那条的教训不在「少认了一种 reason」，而在**判定规则被上游改坏之后，没有任何机制
会察觉**：那张号的 `total_failure_count` 全程是 0，因为当时没有任何计数器会为「不认识
的错误」增长，于是它一直是「可用」状态被反复选中。规则总有再被改坏的一天，兜底闸不能
依赖「我们认得出这个错」。

新增熔断隔离，只看形态不看内容：同一张号连续 5 次确定性失败、期间一次都没成功过，就把
它移出轮转。隔离时长按熔断轮次翻倍、封顶 2 小时，任何一次成功全部清零。

- 只统计**根因不在请求侧**的失败。工具 schema 违规、请求体超长换哪张号都是一样结果，
  算到号头上会让一个坏客户端把整池好号刷进隔离。
- 是隔离不是禁用。走到熔断说明这个错误我们没认出来，也就无从判断它是永久还是临时的，
  只能低频复探、自愈恢复，不需要人工干预。
- 同一形态的故障，烧掉的请求量从「一天 3005 次」降到「几十次」。

顺带把 8 处「跳过冷却中的号」的逐字段判断收敛成 `is_cooling()`——这些判断散落各处，
加一种新冷却只要漏改一处就会静默失效，正是同一类隐患。
...
- 收到 meteringEvent 后，空闲容忍度从 120 秒收到 5 秒；超时即按**正常结束**收尾，
  不再记错误。任何新字节都会重置窗口，所以尾随帧不会被截断。
- 判定**只看 metering**，刻意不再要求「所有 content block 已闭合」。第一版加了那个条件，
  结果整个修复静默失效：Kiro 的文本块只在我们自己收尾时才关，上游从不发对应事件，
  于是流循环里 idle 触发的那一刻 text 块**必然**还开着，分支一次都走不到。
- 终止标记按**轮**重置：续写会另发一次上游请求，新一轮要重新等自己的 meteringEvent。
  不重置的话第 2 轮开局就带着上一轮的标记，整轮只剩 5 秒容忍度，一次真实停顿就会被
  当成正常收尾而静默截断。累计 `credits` 反过来要跨轮保留，只清这一个标记。
- 行为变化：这些请求现在与「上游正常关闭连接」走同一条路径，因此在开启 `autoContinue`
  时也会像正常完成那样参与续写判定（此前它们一律记为 idle timeout，不参与）。
  语义上应当一致——上游发完 metering 后静默，和它关闭连接没有区别。
...
查询 `/api/me/stock` 固定不带 `region`——按对方文档不传就是美区。于是两个需求同时失效：

- **「美区缺货自动买欧区」根本走不到**：查到美区库存 0 → 上层 `select_purchase_count`
  判 0 → `count < min_purchase` → 直接跳过，购买请求压根没发出去。
  `purchase_kiro_drop` 里那条 409→欧区回退成了死代码，它只能救「查到有货、下单前被抢空」
  这个窄缝。线上 `drop` 供应商的事件全是 `skipped`，与此一致。
- **「指定只买欧区」查错了区**：`requested_region` 是欧区，库存却查的美区。
  美区一空就跳过，欧区有 120 也不买；反过来美区有货而欧区空时会去撞一个注定 409 的下单。

现在 Drop 的报价按区走：

| 配置 | 行为 |
|---|---|
| 固定区域（只买欧区） | 只查 `?region=eu`，如实报该区库存；不替运维探别的区 |
| 库存优先 / 不传区域 | 先查默认区（美区）；有货就保持不指定区域下单（留着 409→欧区回退兜住被抢空）；美区为 0 才多问一次欧区，有货就把区带出去，上层据此直接打欧区，不必先撞一个 409 |
| 两区都空 | 报 0，按「都没货」跳过，而不是拿 0 去下单 |

默认区有货时不会多发那次欧区查询（有测试断言），所以常见路径的请求数不变。

### ✅ 适配 — Drop 新版 US/EU 合并到货通知

对方 2026-08 改版把 dual 标记从 `region` 挪到了 `notification_scope`，`region` 改成真实
主区域，并在顶层补上对应主区域的 `purchase_order_id` / `order_id`。这是向后兼容设计，
读顶层三个字段即可购买主区域——我们现有解析**无需改动**即可正确工作，而且比改版前更好：
旧版 `region:"dual"` 解析不出区域（得 `None`，落库靠配置兜底），新版能解析出真区域。

新增两条测试固定这一点：一条用新版通知的**原文**（含 `notification_scope`、`regions`、
`*_by_region` 等我们不读的新字段，确认它们不会让解析失败），一条确认旧版 `"dual"`
字面量仍不会炸——对方可能灰度或回滚。

未做：用 `purchase_order_ids_by_region` 里对方给的**各区幂等号**替换我们换区时自己派生的
那个。我们派生的键是 sha256 确定值，重试稳定、同样不会二次扣费；换成对方的键需要给
事件表加一列来支撑重试持久化，收益不足以换一次 schema 变更。
...
- **压缩信号提前到 85%**：`model_context_window_exceeded` 原先只在上下文占比达到
  **100%** 时下发，属事后通知——那时压缩自身也没有余量了（compact 请求同样带全量
  历史、同样撞上游字节上限），线上实测 240 分钟内信号发了 5 次、会话仍一路死在
  400。改为默认 85% 下发，给压缩留 15% 窗口。
- **新增配置项 `contextWindowSignalThresholdPct`**（默认 `85`）：触发过于频繁可上调至
  88–90，仍撞 400 可下调至 80。非法值（≤0、NaN、∞）退回 100，宁可不触发也不误触发。
- **不改 usage 上报，因此对下游计费零影响**。NewAPI 按 `message_start` 的
  `input_tokens` 扣客户额度，走「把 usage 报成真值」那条路会让客户账单平均翻倍、
  长会话涨约 3.5 倍，故未采用。

### 🔧 修复 — 断流快照终于能存下断点内容

- **`stream_tail` 不再退化为摘要**：原实现只在流尾是合法 UTF-8 时存原文，否则替换成
  sha256 摘要。上游是 AWS event-stream 二进制帧，**永远**过不了 UTF-8 校验，导致线上
  34307 条断流快照的尾部无一例外只剩 120 字节摘要，续写与断流复盘拿不到任何素材。
  现按原样存字节（承载列本就是 BLOB、`content_type` 本就是 `application/octet-stream`）。
- **尾部上限从 256 KB 收到 32 KB**：定位断点只需末尾几帧。
- **只对断流类错误存尾部**（`stream_idle_timeout` / `stream_read_error` /
  `stream_interrupted`），其余类型不再落盘正文。
- **尾部独立 48 小时保留期**：短于快照库自身的保留期。尾部里是模型输出明文，而断流
  排障基本在 1–2 天内完成；到期只删该分片，快照元数据与其余分片保留。

### 🔧 修复 — 错误分类口径统一

- **失败按真实类型入库**：非流式与流式的终态失败原先一律记成 `bad_request`，而 API
  侧返回的是真实类型，导致同一条 schema 违规同时出现在 `bad_request` 与
  `upstream_tool_schema_error` 两个桶里，按 `error_type` 做的统计全部不可信。现改为
  记录真实失败类型，拿不到才退回 `bad_request`。
- ⚠️ **口径变更**：升级前后的 `error_type` 统计不可直接比较，按此字段配置的报表与告警
  阈值需要重新校准。

### ✨ 优化 — 快照存储与日志降噪

- **`client_disconnected` 只采样 1% 存请求体**：该类占快照库 19088 条 / 3.2 GB，而客户端
  主动断开基本不是缺陷。采样按 `trace_id` 哈希做，判定确定性、不引入随机源；元数据仍
  全量保留。
- **历史请求体一并回填清理**：采样只拦新写入，已落库的那 3.2 GB 由维护流程分批剥离
  （每轮上界 256 条，靠 `needs_follow_up` 推进，清完自然停止）。复用同一采样谓词，
  历史与新数据口径一致；`pinned` / `retention_exempt` 的快照一律不动，元数据与诊断
  分片全部保留。
- **thinking 降级日志改为聚合上报**：「客户端请求了 thinking 但上游未返回 reasoning」在
  4.3 小时内出现 13233 次、占全部 WARN 的 70%，把 docker 日志可回溯窗口压到 4 小时。
  改为逐条 DEBUG + 每分钟一行按模型汇总的 INFO，趋势保留、噪音消除。请求本身行为不变。
