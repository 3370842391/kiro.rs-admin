...
├── src/        # 项目 Rust 源码
├── admin-ui/   # 管理面板源码
└── data-test/  # 测试配置、凭据和数据库（被 Git/Docker build 忽略）
```
...
Docker 映射监听所有网卡，并在服务器防火墙放行 TCP 8991。外部检测地址为 `http://43.225.196.10:8991`。本轮不引入域名或 TLS 反向代理；如检测平台强制 HTTPS，再独立增加测试子域名。
...
- 在服务器执行首次构建，记录冷启动耗时并确认 `http://127.0.0.1:8991/` 与公网 `http://43.225.196.10:8991/` 可访问。
