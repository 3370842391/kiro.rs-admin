...
```bash
git add -- src/anthropic/tool_schema.rs src/anthropic/mod.rs src/anthropic/converter.rs src/anthropic/handlers.rs src/anthropic/stream.rs src/bin/anthropic_probe.rs
git commit -m "fix(tool): 校验并修复确定性Schema参数"
```

**客户影响：** 普通 `tool_choice=auto` 且参数合法时无变化；required tool 在完整校验前会缓冲，可能增加少量首包延迟；只有上游生成非法参数时才产生一次额外调用。不会执行错误工具，也不会从客户文本猜值。
