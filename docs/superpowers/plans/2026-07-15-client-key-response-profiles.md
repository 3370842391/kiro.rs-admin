...
- [ ] **Step 4：给 ClientKey 增加字段并让所有创建路径显式初始化**
...
- [ ] **Step 6：运行 Task 1 测试和原客户端 Key 测试**
...
- [ ] **Step 6：运行 Admin API 与客户端 Key 回归**
...
- [ ] **Step 6：运行 trace、快照和 handler 回归**
...
- [ ] 旧 client_api_keys.json 无 responseMode 时全部加载为 detection。
- [ ] 新建和编辑 API 只接受 detection/kiro_native，未知值返回 400。
- [ ] Key 鉴权一次返回 ID、分组、模式快照；在途请求不受后台切换影响。
- [ ] detection 保持现有身份归一化、模型资料和精确回复。
- [ ] kiro_native 不执行身份归一化和检测型本地短路。
- [ ] 两种模式共用工具、thinking、PDF 提取、WebSearch、strict JSON、重试、缓存和计费。
- [ ] 本地 ping 与早期 SSE 未被门控，NewAPI 首响应目标保持 1 秒。
- [ ] trace 与错误快照保存请求发生时的 responseMode，不记录明文 Key。
- [ ] 管理端显示后端实际模式，可创建和编辑，失败时不显示未保存值。
- [ ] Rust、Anthropic probe、Bun 测试和前端生产构建全部通过。
