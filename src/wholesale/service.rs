...
use super::health::{AccountHealth, classify_from_error_text};
use super::store::{Holding, SharedWholesaleStore, holding_status, mother_state};
...
impl WholesaleService {
    ...
    async fn deliver_one(
        ...
        if !health.is_alive() {
            ...
            if health.is_dead() {
                ...
                let _ = self
                    .store
                    .set_mother_state_if_dying(&acc.mother_id, MOTHER_DEATH_THRESHOLD_1H);
            }
        ...
        let (ksk_id, raw_key) = match self
            .token_manager
            .create_ksk_for(acc.credential_id as u64, &label)
            .await
        {
            Ok(v) => v,
            Err(e) => {
    ...
    pub async fn sync_pool(
        ...
        while need > 0 {
            ...
            match self
                .deliver_one(customer_id, region, exclude_mothers, label_prefix)
                .await
            {
                Ok(Some(_h)) => {
                    added += 1;
                ...
                Ok(None) => {
                    // 池空
                    shortfall = Some(Shortfall {
                        reason: "pool_exhausted".to_string(),
                        missing: Some(need),
                    });
                    break;
                }
    ...
    pub fn current_pool(&self, customer_id: i64, with_key: bool) -> Result<Vec<PoolItem>, String> {
        let holdings = self
            .store
            .holdings_for_customer(customer_id, false)
            .map_err(|e| e.to_string())?;
        Ok(holdings
            .into_iter()
            .map(|h| Self::to_pool_item(h, with_key))
            .collect())
    }
        ...
        for (holding_id, credential_id, mother_id, _customer_id, _warranty, _status) in targets {
            ...
            match health {
                ...
                AccountHealth::LowQuota => {
                    let _ = self.store.update_holding_probe(
                        holding_id,
                        holding_status::LOW_QUOTA,
                        None,
                    );
                }
                AccountHealth::Active => {
                    let _ =
                        self.store
                            .update_holding_probe(holding_id, holding_status::ACTIVE, None);
                }
